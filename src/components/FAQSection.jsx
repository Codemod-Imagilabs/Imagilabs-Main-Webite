...
const FAQSection = () => {
  ...
  return (
    <section className="w-full py-32 bg-transparent relative overflow-hidden z-10">
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex flex-col lg:flex-row gap-20">
        ...
        <div className="flex-1 space-y-4">
          {faqs.map((faq, index) => (
            <div 
              key={index} 
              className="border-b border-white/[0.1] last:border-none"
            >
              <button
              ...
              >
                <div className={`flex-shrink-0 transition-all duration-500 ease-in-out ${openIndex === index ? 'rotate-45' : 'rotate-0'}`}>
                  <Plus className={`w-6 h-6 transition-colors duration-300 ${openIndex === index ? 'text-white' : 'text-white/40 group-hover:text-white'}`} />
                </div>
                <span className={`text-xl md:text-2xl font-medium transition-colors duration-300 ${openIndex === index ? 'text-white' : 'text-white/60 group-hover:text-white'}`}>
                  {faq.question}
                </span>
              </button>
        ...
        <div className="lg:w-[450px] space-y-12">
          <h2 className="text-5xl md:text-6xl font-normal tracking-tight leading-tight">
            Frequently Asked Questions
          </h2>
          
          {/* Image Placeholder (Empty/Gray) */}
          <div className="aspect-[4/3] bg-white/[0.03] rounded-3xl border border-white/[0.08] backdrop-blur-xl relative overflow-hidden">
